...
ssh "$PI_HOST" "sed -i 's/\r$//' ${REMOTE_DIR}/scripts/build-on-pi.sh ${REMOTE_DIR}/scripts/deploy-to-pi.sh ${REMOTE_DIR}/scripts/install-pi-service.sh"
...
echo "Run on Pi (waits for ESP START on GPIO4 by default):"
echo "  ssh $PI_HOST \"${REMOTE_DIR}/build-rpi/mars-cv --dual --model ${REMOTE_DIR}/models/${ACTIVE_MODEL}\""
echo "Lab without handshake:"
echo "  ssh $PI_HOST \"${REMOTE_DIR}/build-rpi/mars-cv --dual --model ${REMOTE_DIR}/models/${ACTIVE_MODEL} --no-esp-handshake\""
echo "See cv-dev/ESP32-GPIO-HANDSHAKE.md for the pin contract."
